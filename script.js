...
// 9. Lazy Loading للصور مع تأثير الظهور التدريجي
function initLazyLoading() {
    // تحديد جميع الصور التي تحتاج lazy loading
    ...
    const imageObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // إذا كانت الصورة لها data-src attribute، قم بتحميلها
                if (img.dataset.src) {
                    img.src = img.dataset.src;
                    img.removeAttribute('data-src');
                }
                
                // إضافة class للصورة عندما تظهر
                img.classList.add('lazy-loaded');
                img.setAttribute('data-loaded', 'true');
                
                // إزالة الصورة من المراقبة
                observer.unobserve(img);
                
                // تأثير الظهور التدريجي للصورة
                setTimeout(() => {
                    img.style.opacity = '1';
        ...
        rootMargin: '50px', // يبدأ التحميل قبل 50px من ظهور الصورة
        threshold: 0.1 // 10% من الصورة مرئية
    });
    
    // مراقبة جميع الصور
    lazyImages.forEach(img => {
        // تعيين opacity و transform ابتدائي
        img.style.opacity = '0';
...
// تشغيل Lazy Loading للصور الجديدة التي تضاف ديناميكياً
...
// مراقبة DOM للصور الجديدة (مفيد إذا كنت تضيف صوراً ديناميكياً)
const domObserver = new MutationObserver(observeNewImages);
domObserver.observe(document.body, { childList: true, subtree: true });

// 10. تحسين تحميل الصور للـ Hero image أولاً
function preloadHeroImage() {
    const heroImage = document.querySelector('.hero-image img');
    if (heroImage) {
        // نزيل lazy loading من صورة الهيرو لأنها أول ما يظهر
        heroImage.loading = 'eager';
        heroImage.classList.add('priority-load');
    }
}

// تشغيل تحميل صورة الهيرو أولاً
