...
// تطبيق خاصية التحميل الكسول (Lazy Loading) للصور
function initLazyLoading() {
    // تحديد الصور التي لم يتم تحميلها بعد
    const lazyImages = document.querySelectorAll('img:not([data-loaded])');
    
    // استخدام Intersection Observer لمراقبة الصور
    const imageObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const img = entry.target;
                
                // استبدال المصدر بالمصدر الحقيقي
                if (img.dataset.src) {
                    img.src = img.dataset.src;
                    img.removeAttribute('data-src');
                }
                
                // إضافة كلاس لتمييز الصورة المحملة
                img.classList.add('lazy-loaded');
                img.setAttribute('data-loaded', 'true');
                
                // إيقاف المراقبة للصورة
        ...
        rootMargin: '50px', // هامش التحميل المسبق
        threshold: 0.1 
    });
...
// تشغيل التحميل الكسول عند التحميل
window.addEventListener('load', function() {
...
// استخدام MutationObserver لمراقبة تغييرات الـ DOM
